...
//to set the baud rate we have to configure BRR register
...
void USART_PeripheralControl(USART_RegDef_t *pUSARTx, uint8_t Cmd)
{
	if(Cmd == ENABLE)
	{
		pUSARTx->CR1 |= (1 << 0);//SET UE bit
	}else
	{
		pUSARTx->CR1 &= ~(1 << 0); //CLEAR UE bit
	}
...
uint8_t USART_GetFlagStatus(USART_RegDef_t *pUSARTx, uint8_t StatusFlagName)
{
    ...
    {
    	ISR_FLAG = pUSARTx->ISR & StatusFlagName;
...
uint8_t USART_SendDataIT(USART_Handle_t *pUSARTHandle,uint8_t *pTxBuffer, uint32_t Len)
{
	...
	if(txstate != USART_BUSY_IN_TX)
	{
		...
		//Enable TXEIE so that TXE will be set whenever the transmit buffer gets empty
		pUSARTHandle->pUSARTx->CR1 |= ( 1 << USART_CR1_TXEIE);


		//Now set the TCIE in CR1 so that TC will be set whenever a byte transmission
...
{
	//set the IRQ number in NVIC ISER
	...
	if(temp1 && temp2 )
	{
		...
		if ( pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
		{
			...
			if(! pUSARTHandle->TxLen )
			{
				...
				//Implement the lear the TCIE control bit
				pUSARTHandle->pUSARTx->ISR &= ~(1 << USART_iSR_TCIE);
...
/*************************Check for TXE flag ********************************************/
	...
	//Implement the code to check the state of TXE bit in the ISR
	...
	if(temp2 )
	{
		...
		if(temp1 & ( 1 << USART_ISR_FE))
		{
			/*
				...
				is detected. It is cleared by a software sequence (an read to the USART_ISR register
		...
		if(temp1 & ( 1 << USART_ISR_FE) )
		{
			/*
				...
				software sequence (an read to the USART_ISR register followed by a read to the
				USART_DR register).
			*/
